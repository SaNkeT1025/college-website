...
<?php
    ...
    $conn=new mysqli('localhost','root','','college');
        ...
        $sql = "select * from `faculty` where `email` = '$femail'";
...
if(isset($_POST['btnsubmit']))
{
    ...
    if($_FILES["fileToUpload"]["name"])
    {
        ...
        else
        {
            ...
            $sql = "UPDATE `faculty` SET `profile_pic`='$file', `fname`='$fname',`mname`='$mname',`lname`='$lname',
                    `department`='$department',`designation`='$designation',`address`='$address',`email`='$email',`mb`='$mobileno' WHERE `email`='$femail'";
            $flg=1;
        }
    ...
    {
        $sql = "UPDATE `faculty` SET `fname`='$fname',`mname`='$mname',`lname`='$lname',
        `department`='$department',`designation`='$designation',`address`='$address',`email`='$email',`mb`='$mobileno',`password`='$newpass' WHERE `email`='$femail'";
        $flg=2;
}
    $result = $conn->query($sql);
    if($result)
    {
        if($flg==1)
        {
            $sql="UPDATE chatusers SET fname='$fname',lname='$lname',profilename='$file' WHERE `prn`='$facultyid'";
        }
        else
        {
        ...
        $result = $conn->query($sql);
        $_SESSION["username"]=$email.' '.'f';
        echo "<script>
        alert('profile Updated Successfully!!');
        window.location.href='./facultyprofile.php?id=';
        </script>";
    }
    else
    {
        ...
        window.location.href='./profileupdate.php?id=';
        </script>";
    }
}
