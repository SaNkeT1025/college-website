...
<center>
    <div class="container">
        <form action="syllabus.php" method="post" enctype="multipart/form-data">
            <div class="mb-3">
                <p class="h1" style="color:black ; background-color:#1d96ff ;margin-top:48px; ">Upload Syllabus Here </p>
            </div>
...
<?php
    ...
    {  
        ...
        $conn = mysqli_connect ('localhost', 'root', '', 'college') or die(mysqli_error());
            ...
            else if(file_exists("../student/syllabus/".$_FILES['pdf1']['name']))
            {
            ...
            $sql = "INSERT INTO syllabus (`pdf`,`type`) VALUES ('$pdfname','$type')";
            $run = mysqli_query($conn, $sql);
            if($run)
            {
                move_uploaded_file( $temp_pdf_name, "../student/syllabus/$pdfname");
...
<center>
 ...
 margin-top:20px;" onclick="window.location.href='syllabusdetails.php'">View Uploaded Syllabus</button>
</center>