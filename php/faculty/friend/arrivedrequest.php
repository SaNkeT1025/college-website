...
<body style="height:calc(100vh);background-image:url(../../images/p5-dark.jpg);background-size:cover;background-attachment: fixed;">
        ...
        <?php
            ...
            $query = "UPDATE requests SET read_status=1 where friendid='$id'";
            $result = $connection->query($query);
            $query = "SELECT * from faculty where facultyid in(select user from requests where friendid='$id' and status='pending')";
            $result = $connection->query($query);
            ?>
            ...
            <?php
                $query1 = "SELECT * from student where prn in(select user from requests where friendid='$id' and status='pending')";
