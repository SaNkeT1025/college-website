...
<body style="height:calc(100vh);background-image:url(../../images/p5-dark.jpg);background-size:cover;background-attachment: fixed;">
        ...
        <?php
            ...
            $query = "SELECT * from student where prn in(select friendid from requests where user='$sprn' and status='pending')";
            ...
            <?php
                $query = "SELECT * from faculty where facultyid in(select friendid from requests where user='$sprn' and status='pending')";
                $result = $connection->query($query);
            ?>
